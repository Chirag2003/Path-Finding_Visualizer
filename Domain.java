...
public class Domain {
	...
	// AO* Algorithm
	public void AOStar(Node start, Node end) {
        System.out.println("Performing AO* Search Algorithm.");
                ...
                if (!pqueue.isEmpty()) {
                    ...
                    if (!current.visited) {
                        ...
                        for (int i = 0; i < current.edges.size(); i++) {
                            ...
                            if (!closedSet.contains(edge) && !edge.isObstacle) {
                                double newCost;
                                if (current.isDiagonal(edge))
                                    newCost = current.g + 1.4;
                                else
    ...
    // Hill Climbing
    public void HillClimbing(Node start, Node end) {
        System.out.println("Performing Hill Climbing Algorithm.");
        ...
        while (!stack.isEmpty()) {
            ...
            for (int i = 0; i < current.edges.size(); i++) {
                Node neighbor = current.edges.get(i);

                if (!neighbor.visited && !neighbor.isObstacle) {
                    double neighborHeuristic = neighbor.getDistance(end);
